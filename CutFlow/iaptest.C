...
void iaptest(const TString infilename="/afs/cern.ch/work/k/klawhorn/test/HHToGGWW_14TeV.root") {
  ...
  TH1D *testMass = new TH1D("testMass", "testMass", 100, 0, 1000);
  ...
  for(UInt_t iEntry=0; iEntry<intree->GetEntries(); iEntry++) { // entry loop
    ...
    if ( (tauCat1 == hadron) || (tauCat2 == hadron) ) continue;

    LorentzVector vHiggs = *recoTau1 + *recoTau2;

    testMass->Fill(vHiggs.M());
