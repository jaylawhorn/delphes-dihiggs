...
void lep_selection(const TString inputfile="/afs/cern.ch/work/k/klawhorn/public/HHToGGWW_14TeV/HHToGGWW_14TeV_0.root",
  ...
  outTree->Branch("recoHiggs",      "ROOT::Math::LorentzVector<ROOT::Math::PtEtaPhiM4D<double> >", &recoHiggs);    // 4-vector for reconstructed higgs
  ...
  for (Int_t iEntry=0; iEntry<numberOfEntries; iEntry++) { // entry loop
    ...
    LorentzVector vHiggs = vRecoTau1+vRecoTau2;
    recoHiggs = &vHiggs;
