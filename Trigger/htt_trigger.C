...
void htt_trigger(const TString input="htt.txt",
  ...
  for (Int_t iEntry=0; iEntry<n; iEntry++) { // entry loop                                                                          
    ...
    for (Int_t i=0; i<branchParticle->GetEntries(); i++) {
      ...
      //if (fabs(part->PID == ELE_ID_CODE) ) cout << part->M1 << " " << part->M2 << endl;
      ...
      if (fabs(part->PID) == ELE_ID_CODE && part->PT > e1pt && part->M1 < 20) {
	//cout << part->M1 << endl;
	Int_t foo=part->M1;
	if (foo < branchParticle->GetEntries() ) {
	  mama = (GenParticle*) branchParticle->At(foo);
	  //cout << "mother is " << mama->PID << endl;
	  if (abs(mama->PID)<11 || abs(mama->PID)>16) continue;
	}
	e2pt=e1pt;
	e2eta=e1eta;
	e2phi=e1phi;
	...
	e1phi=part->Phi;
      ...
      else if (fabs(part->PID) == ELE_ID_CODE && part->PT > e2pt && part->M1 < 20) {
	//cout << part->M1 << endl;
	Int_t foo=part->M1;
	if (foo < branchParticle->GetEntries() ) {
	  mama = (GenParticle*) branchParticle->At(foo);
	...
	e2phi=part->Phi;
      ...
      else if (fabs(part->PID) == MU_ID_CODE && part->PT > m1pt && part->M1 < 20) {
	//cout << part->M1 << endl;
	Int_t foo=part->M1;
	if (foo < branchParticle->GetEntries() ) {
	  mama = (GenParticle*) branchParticle->At(foo);
	...
	m1phi=part->Phi;
      ...
      else if (fabs(part->PID) == MU_ID_CODE && part->PT > m2pt && part->M1 < 20) {
	//cout << part->M1 << endl;
	Int_t foo=part->M1;
	if (foo < branchParticle->GetEntries() ) {
	  mama = (GenParticle*) branchParticle->At(foo);
	...
	m2phi=part->Phi;
      ...
      else if (fabs(part->PID) == TAU_ID_CODE && part->Status==3 && part->PT > t1pt ) {
	t2pt=t1pt;
	t2eta=t1eta;
	t2phi=t1phi;
	t1pt=part->PT;
	...
	t1phi=part->Phi;
      ...
      else if (fabs(part->PID) == TAU_ID_CODE && part->Status==3 && part->PT > t2pt) {
	t2pt=part->PT;
	t2eta=part->Eta;
	t2phi=part->Phi;
      }
